/*
59.Write a program which accepts two singly linear linked list from user and compare both
linked list but only last N nodes.
Function Prototype : int ListICmp(struct node *SrcHead struct node *SrcTail, struct node*DestHead, struct node *DestTail, int N );
...
//----------------------------ListNCmp function ------------------------
int ListICmp(node *srcHead , node *srcTail, node *destHead, node *destTail , int n) {
		
		node *tempSrc = srcHead , *tempDest = destHead;		//take two temp pointers
		int srcPos = getLength(srcHead) - n;			//calculate pos for staring
		int destPos = getLength(destHead) - n;			// comparision

		int max = srcPos > destPos ? srcPos : destPos;		//find maximum
...
int main() {
	...
	while(pos > min || pos <= 0) {
		if( pos <= 0) 
			std::cout << "You have entered negative value or ZERO . Enter Positiove only " << std::endl;
		else 
	...
	int status = ListICmp(src,tailSrc,dest,tailDest, pos);
	if(status == 0)
		std::cout << "Last " << pos << " nodes in the List are Same "  <<std::endl;
	else if(status == 1)
		std::cout << "Last " << pos << " nodes in the List are not Same "  <<std::endl;
