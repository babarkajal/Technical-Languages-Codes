/*
49.Write a program which search last occurrence of particular element from singly circular
linked list.
...
int SearchLastOcc(struct node *head, struct node *tail, int no) {
		...
		do {
			...
			if(temp->no == no) {
				pos = count;	//store current pos of element
			}
...
int main() {
	...
	std::cin >> num;
	//call funciton
	int status = SearchLastOcc(head ,tail , num);

	//check found or not
	if(status) 
		std::cout << "Last occurance of " << num << " is found " << "at position "<< status <<std::endl; 
	else 
		std::cout <<  num << " is not found in the list " << std::endl; 
}
