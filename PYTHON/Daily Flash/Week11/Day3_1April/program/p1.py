...
Series: (x+y)/1! + 2*(x+y)/2! + 3*(x+y)/3! + ... + n(x+y)/n! ( Where: x & y are the numbers entered by user)
Input: Enter Values of x & y = 2 4
Enter Length of Series: 3
Output: The Addition of entered Series: 8.5
'''
...
def add(num,x,y):
    ...
    while value <= num:
            add_of_series = add_of_series +  (value*(x+y) )/ fact(value)
            value += 1
